...
export default function AdminPromptsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => apiClient.get(`/admin/prompts?page=${page}&limit=20`).then(r => r.data),
  });
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        ...
        <Link href="/admin/prompts/new" className="btn btn-primary">{t.admin.create_prompt}</Link>
      </div>
              ...
              <tbody>
                {data.data.map((p: any) => (
                  <tr key={p.id}>
                    ...
                    <td>
                      <Link href={`/admin/prompts/${p.id}/edit`} className="btn-icon" title={t.common.edit}>✏️</Link>
