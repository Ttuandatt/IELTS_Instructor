...
export default function PassageDetailPage() {
    ...
    const updateQuestionMut = useMutation({
        mutationFn: (payload: { qid: string, answer_key: string }) => apiClient.patch(`/instructor/questions/${payload.qid}`, { answer_key: payload.answer_key }),
        onSuccess: () => {
    ...
    const handleDelete = async () => {
        ...
        try {
            await apiClient.delete(`/instructor/passages/${id}`);
            toast.success('Passage deleted');
            router.push('/instructor/passages');
        } catch (err: any) {
            toast.error(err.response?.data?.message || 'Failed to delete');
        }
    };

    const isOwner = user?.role === 'admin' || data?.created_by === user?.id;
            ...
            <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-3 mt-1 text-sm">
                            ...
                            {data.creator && <span className="text-gray-400">by {data.creator.display_name}</span>}
                        </div>
                    </div>
                </div>
                ...
                {isOwner && (
                    <div className="flex gap-3">
                        <Link href={`/instructor/passages/${id}/edit`} className="flex items-center gap-2 px-4 py-2 border rounded-lg text-gray-700 hover:bg-gray-50">
